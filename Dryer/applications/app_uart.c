...
{
    /* 发送事件 */
    rt_event_send(&_event, UART_RX_EVENT);
...
rt_uint8_t uart_getchar(rt_device_t uart_device)
{
    ...
    while (rt_device_read(uart_device, 0, &ch, 1) != 1){
         /* 接收事件 */
        rt_event_recv(&_event, UART_RX_EVENT,RT_EVENT_FLAG_AND | RT_EVENT_FLAG_CLEAR,RT_WAITING_FOREVER, &e);
    }

    return ch;
}
void uart_putchar(rt_device_t uart_device, const char c)
{
    rt_size_t len = 0;
...
void uart_putstring(rt_device_t uart_device, const char *s)
{
	if(RT_NULL == uart_device){
		return;
	}
    while(*s){
        uart_putchar(uart_device, *s++);
    }
}
...
rt_device_t uart_open(const char *name)
{
    ...
    if (uart_device != RT_NULL){   
		...
		/* 初始化事件对象 */
		rt_event_init(&_event, "event", RT_IPC_FLAG_FIFO); 
    }else{
...
rt_err_t uart_close(rt_device_t uart_device)
{
	rt_err_t res = RT_ERROR;
	if(RT_NULL == uart_device){
		return res;
	}
	res = rt_device_close(uart_device);
	return res;
}
